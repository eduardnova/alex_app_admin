...
def get_cipher():
    ...
    if not key:
        # Este error detendrÃ¡ la aplicaciÃ³n al iniciar si falta la clave.
        raise ValueError("FERNET_KEY no estÃ¡ configurada en la aplicaciÃ³n")
    ...
    try:
        ...
        raise ValueError(f"FERNET_KEY invÃ¡lida: {str(e)}")


def encrypt_data(data):
    """Cifra datos sensibles (como CÃ©dula, Licencia, TelÃ©fono)"""
    ...
    try:
        ...
        # Propaga el error para detener la operaciÃ³n si falla la encriptaciÃ³n
        raise
...
def decrypt_data(encrypted_data):
    ...
    try:
        cipher = get_cipher()
        # El descifrado puede fallar si la clave es incorrecta o el token estÃ¡ corrupto
        return cipher.decrypt(encrypted_data.encode()).decode()
    except InvalidToken: 
        # Error especÃ­fico: La clave FERNET_KEY es incorrecta o el dato cifrado estÃ¡ corrupto
        print("Error al desencriptar: Clave incorrecta (FERNET_KEY) o dato cifrado invÃ¡lido.")
        # Es crucial forzar la reversiÃ³n (ROLLBACK) si no se puede leer un dato crÃ­tico.
        raise
    except Exception as e:
        # Para cualquier otro error (ej. problema con la codificaciÃ³n)
...
class Usuario(UserMixin, db.Model):
    ...
    def check_password(self, password):
        """Verify password"""
        if not self.password or self.password == '':  # â† DEBE ESTAR AQUÃ
            return False
...
"""
Database Models - Part 2: Propietarios, Inquilinos, VehÃ­culos, MecÃ¡nicos, Alquileres
"""
...
class Propietario(db.Model):
    ...
    # Campos encriptados (YA ESTABAN CORRECTOS con mapeo explÃ­cito)
    ...
    @property
    def tipo_socio(self):
        """Calcula el tipo de socio segÃºn cantidad de vehÃ­culos"""
    ...
    def badge_socio(self):
        """Retorna el badge CSS segÃºn el tipo de socio"""
...
# Nota: Se asume que encrypt_data y decrypt_data estÃ¡n definidos
...
class Vehiculo(db.Model):
    ...
    # CORRECCIÃ“N: Mapeo explÃ­cito a la columna 'placa' de la base de datos
    ...
    # CORRECCIÃ“N: Mapeo explÃ­cito para el resto de columnas encriptadas
...
class Alquiler(db.Model):
    ...
    def __repr__(self):
        return f'<Alquiler {self.id} - VehÃ­culo {self.vehiculo_id}>'
...
class PorcentajeGanancia(db.Model):
    ...
    que se aplican al calcular la nÃ³mina en los alquileres
    """
...
class SemanaAlquiler(db.Model):
    ...
    Cada registro representa una semana completa con su configuraciÃ³n
    """
    ...
    numero_semana = db.Column(db.Integer)  # NÃºmero de semana del aÃ±o
    ...
    def dias_semana(self):
        """Calcula los dÃ­as de la semana"""
...
class DetalleAlquilerSemanal(db.Model):
    """
    Tabla detallada de cada alquiler en una semana especÃ­fica
    Contiene toda la informaciÃ³n por vehÃ­culo/inquilino en esa semana
    """
    ...
    def esta_en_mora(self):
        """Verifica si el pago estÃ¡ en mora"""
...
class HistoricoPorcentajeGanancia(db.Model):
    """Tabla histÃ³rica para porcentajes de ganancia"""
...
class HistoricoSemanaAlquiler(db.Model):
    """Tabla histÃ³rica para semanas de alquiler"""
...
class HistoricoUsuario(db.Model):
    ...
    # Campos histÃ³ricos del usuario
    id = db.Column(db.Integer)
