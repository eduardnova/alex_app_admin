// ==========================================
// ALQUILERES.JS - SISTEMA COMPLETO
// Sistema de Gestión de Alquileres Semanales
// Versión: 2.0.0 - Con Sistema de Inversiones
// ==========================================
...
function toggleSemana(semanaId) {
    ...
    if (!details || !toggle) {
        console.error(`❌ No se encontraron elementos para semana ${semanaId}`);
        return;
    }
    ...
    detalles.forEach((detalle, index) => {
        ...
        row.insertCell().innerHTML = `
            <select class="excel-select" data-field="banco_id" onchange="marcarCambio(${detalle.id}, ${semanaId})">
                ...
                ${window.bancosGlobal ? window.bancosGlobal.map(b => 
                    `<option value="${b.id}" ${detalle.banco_id == b.id ? 'selected' : ''}>${b.banco}</option>`
                ).join('') : ''}
            </select>
        `;
...
function gestionarInversiones(detalleId, semanaId) {
    ...
    const detalle = semanasCacheIndependiente[semanaId]?.find(d => d.id === detalleId);
...
// ==========================================
// CREAR MODAL DE INVERSIONES
// ==========================================
function crearModalInversiones(detalle, semanaId) {
    const modalHTML = `
        <div class="modal-overlay" id="inversionesModal">
            <div class="modal modal-lg">
                ...
                <div class="modal-body">
                    <div class="alert alert-info" style="margin-bottom: 20px;">
                        ...
                        <div>
                            <strong>Precio Semanal:</strong> $${detalle.precio_semanal.toFixed(2)}
                            ${detalle.inversion_mecanica >= detalle.precio_semanal ? 
                            ...
                            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
                                <div class="form-group">
                                    ...
                                    <select name="tipo_trabajo_id" class="form-select" required>
                                        <option value="">Seleccionar...</option>
                                        ${window.tiposTrabajo ? window.tiposTrabajo.map(t => `
                                            <option value="${t.id}">
                                                ${getIconoTipoTrabajo(t.nombre)} ${t.nombre}
                                            </option>
                                        `).join('') : ''}
                                    </select>
                                </div>
                    ...
                    <!-- Tabla de Inversiones Existentes -->
                    <div>
                        ...
                        <div class="table-responsive">
                            <table class="table" id="tablaInversiones">
                                ...
                                <tbody>
                                    ${detalle.inversiones && detalle.inversiones.length > 0 ? 
                                        detalle.inversiones.map(inv => `
                                            <tr>
                                                <td><span class="badge badge-warning">${inv.tipo_trabajo}</span></td>
                                                <td>${inv.descripcion}</td>
                                                <td class="text-right"><strong>$${inv.costo.toFixed(2)}</strong></td>
                                                <td>${inv.fecha}</td>
                                                <td>
                                ...
                                <tfoot>
                                    <tr style="background: var(--bg-primary); font-weight: bold;">
                                        ...
                                        <td class="text-right" style="color: ${detalle.inversion_mecanica >= detalle.precio_semanal ? '#856404' : 'inherit'};">
...
function mostrarAdvertenciaSemanas(data) {
    const alertHTML = `
        <div class="alert alert-warning" id="alertSemanasActivas" style="margin-bottom: 20px; animation: slideDown 0.3s ease;">
            ...
            <div>
                ...
                <ul style="margin-top: 8px; margin-bottom: 8px;">
                    ${data.semanas_fuera_rango.map(s => 
                        `<li>Semana #${s.numero_semana}: ${s.fecha_inicio} - ${s.fecha_fin}</li>`
                    ).join('')}
                </ul>
...
function cargarTiposTrabajo() {
    // Ya están cargados en window.tiposTrabajo desde el template
...
function eliminarSemana(semanaId, totalVehiculos) {
    ...
    // Confirmación adicional si hay vehículos
    if (totalVehiculos > 0) {
                ...
                if (vehiculoSelect) {
                    vehiculoSelect.innerHTML = '<option value="">Seleccionar vehículo...</option>' +
                        data.vehiculos.map(v => `
                            <option value="${v.id}" data-precio="${v.precio_semanal}">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (formAgregarAlquiler) {
        formAgregarAlquiler.addEventListener('submit', (e) => {
            e.preventDefault();
...
function editarDetalleCompleto(detalleId, semanaId) {
    ...
    fetch(`/alquiler/semanas/${semanaId}/disponibles`)
        ...
        .then(data => {
            if (data.success) {
                // Agregar el vehículo e inquilino actual a las opciones
                ...
                document.querySelector('#editarDetalleForm [name="inversion_mecanica"]').value = detalle.inversion_mecanica || 0;
                document.querySelector('#editarDetalleForm [name="concepto_inversion"]').value = detalle.concepto_inversion || '';
                document.querySelector('#editarDetalleForm [name="monto_descuento"]').value = detalle.monto_descuento || 0;
                document.querySelector('#editarDetalleForm [name="concepto_descuento"]').value = detalle.concepto_descuento || '';
                document.querySelector('#editarDetalleForm [name="monto_deuda"]').value = detalle.monto_deuda || 0;
                document.querySelector('#editarDetalleForm [name="banco_id"]').value = detalle.banco_id || '';
                document.querySelector('#editarDetalleForm [name="fecha_confirmacion_pago"]').value = detalle.fecha_confirmacion_pago || '';
                document.getElementById('pagoConfirmadoCheck').checked = detalle.pago_confirmado;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (formEditarDetalle) {
        formEditarDetalle.addEventListener('submit', (e) => {
            e.preventDefault();
            
            const formData = new FormData(formEditarDetalle);
...
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    window.debugAlquileres = {
        ...
        limpiarCache: () => {
            Object.keys(semanasCacheIndependiente).forEach(key => {
                delete semanasCacheIndependiente[key];
            });
            console.log('🧹 Cache limpiado');
        },
        recargarSemana: (semanaId) => {
            semanasCacheIndependiente[semanaId] = null;
            cargarDetallesSemana(semanaId);
...
console.log('📊 Sistema de Alquileres v2.0.0 - Con Inversiones Mecánicas');